...
def train_run(net: nn.Module, run_dir_name: str, dataset, num_epochs: int = 100, 
    ...
    for epoch in range(num_epochs):
        ...
        # so there is no concern of overfitting. Leaving this here in case I change my mind later.
        # with torch.no_grad():
        #     val_output_logits = net(val_inputs)
        #     val_loss = criterion(val_output_logits, val_labels)
        #     val_probs = nn.functional.softmax(val_output_logits, dim=1)
...
def train_light_gru():
    ...
    train_run(net, 'light_GRU_run1', dataset, num_epochs=1000, learning_rate=0.001, betas=(0.9, 0.999))

def train_enu_light_gru():
    dataset = initialize_dataset(batch_size=32, task='DualDelayMatchSample-v0')
    net = initialize_model(hidden_size=64, model_type='enu_light', unit_input_size=CONFIG['input_size'])
    train_run(net, 'enu_light_GRU_run1', dataset, num_epochs=1000, learning_rate=0.001, betas=(0.9, 0.999))

def train_light_gru_l2():
    dataset = initialize_dataset(batch_size=32, task='DualDelayMatchSample-v0')
    net = initialize_model(hidden_size=64, model_type='light')
    train_run(net, 'light_GRU_l2_reg', dataset, num_epochs=1000, learning_rate=0.001, betas=(0.9, 0.999), l2_activation_coef=0.001)

def train_ie_light_gru():
    dataset = initialize_dataset(batch_size=32, task='DualDelayMatchSample-v0')
    net = initialize_model(hidden_size=64, model_type='ei_light', e_prop=0.75)
    train_run(net, 'ei_light_GRU_run1', dataset, num_epochs=1000, learning_rate=0.001, betas=(0.9, 0.999))
